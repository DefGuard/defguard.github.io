...
This is where you can manage your users and groups directly. It isn't a premium module or extension you have to buy. [Learn more about Defguard as an SSO provider →](https://docs.defguard.net/features/openid-connect)

#### 2. Built-in Multi-Factor Authentication (MFA)

Defguard handles MFA at the core level. It works with standard authenticator apps like Google Authenticator or Microsoft Authenticator (TOTP-based), so every user can enable MFA without extra licenses or proprietary tokens.
...
It uses a standard called OIDC (OpenID Connect) to securely connect to them as part of its enterprise capabilities. This lets your users log in with the accounts they already have. [Learn more about external SSO integration →](https://docs.defguard.net/features/external-openid-providers)
...
[Learn more in Fortinet VPN MFA setup guide →](https://docs.fortinet.com/document/fortigate/latest/administration-guide/fortitoken)

**What is a good FortiToken alternative?**

A modern alternative to token-based MFA systems. Defguard includes Multi-Factor Authentication as a built-in feature — supporting standard TOTP codes from authenticator apps (like Google Authenticator) and a native user database (IdP) in every deployment.

There's no need for separate hardware or mobile tokens. [Learn more about Defguard VPN with built-in MFA →](https://defguard.net/vpn_mfa/)
