...
### MSS Clamping (Alternative to MTU Adjustment)
...
```
Problem: User on DSL/Fiber with PPPoE cannot load large pages
...
```
Problem: Laptop user reports intermittent failures on mobile hotspot
...
```
Problem: User behind carrier-grade NAT has connectivity issues
...
```
Problem: User behind corporate proxy/firewall drops large packets
