...
const FeaturesTable = ({ data }: TableProps) => {
  ...
  const gridConfig = { '--table-cols': `repeat(${data.others.length + 1}, 140px)`} as CSSProperties;

  return (
    <section class="features-table" style={gridConfig}>
      <header>
        <div class="col-header left"><p>Feature</p></div>
        <div class="col-header">
          <FeaturesTableDefguardLogo />
        </div>
        {data.others.map((o) => (
          <div class="col-header"><p>{o}</p></div>
        ))}
      </header>
      <div class="table-rows">
        {data.groups.map((g, groupIndex) => (
          <Fragment key={groupIndex}>
            ...
            {g.rows.map((row) => (<div class="row" key={row.description.toLowerCase().trim().replace(' ', '')} >
                ...
                <div class="status-container" style={{
                  ...
                  gridColumn: `2 / ${featuresColCount + 2}`
                    ...
                    {row.features.map((f, index) => (<FeaturesTableCell hover={f.hover} link={f.link} status={f.status} key={index} />))}
                </div>
            </div>))}
          </Fragment>
        ))}
